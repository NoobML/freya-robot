...
// Animation - eyes thinking: alternating eye size change
bool thinking = 0;
unsigned long thinkingAnimationTimer = 0;
int thinkingAnimationInterval = 400; // duration for each step of the alternation in milliseconds
bool thinkingToggle = 0;

// Variables for default animation reset (run once)
bool defaultAnimation = 0;
unsigned long defaultAnimationToggle = 0;
...
// Play thinking animation - continuously alternates left and right eye sizes
void anim_thinking() {
    thinking = 1;
}
...
void drawEyes(){
  ...
  // Thinking - alternating eye size change
  if (thinking) {
    if (millis() >= thinkingAnimationTimer + thinkingAnimationInterval) {
      thinkingAnimationTimer = millis(); // Reset timer

      if (thinkingToggle) {
        // Left eye grows, right eye shrinks
        eyeLheightNext = eyeLheightDefault + 6; // Left eye grows larger
        eyeLwidthNext = eyeLwidthDefault + 4;  // Maintain aspect ratio

        eyeRheightNext = eyeRheightDefault - 6; // Right eye shrinks
        eyeRwidthNext = eyeRwidthDefault - 4;  // Maintain aspect ratio
      } else {
        // Right eye grows, left eye shrinks
        eyeLheightNext = eyeLheightDefault - 6; // Left eye shrinks
        eyeLwidthNext = eyeLwidthDefault - 4;  // Maintain aspect ratio

        eyeRheightNext = eyeRheightDefault + 6; // Right eye grows larger
        eyeRwidthNext = eyeRwidthDefault + 4;  // Maintain aspect ratio
      }

      thinkingToggle = !thinkingToggle; // Alternate toggle
    }
  }

  if (defaultAnimation) {
      // Reset to default eye sizes
      eyeLheightNext = eyeLheightDefault; // Left eye default height
      eyeLwidthNext = eyeLwidthDefault;   // Left eye default width

      eyeRheightNext = eyeRheightDefault; // Right eye default height
      eyeRwidthNext = eyeRwidthDefault;   // Right eye default width

      // Reset thinking animation
      thinking = 0;        // Stop thinking animation
      thinkingToggle = 0;  // Reset toggle for alternating eyes
      thinkingAnimationTimer = millis(); // Reset animation timer to prevent instant trigger

      // Set the flag to prevent re-running the function
      defaultAnimation = 0;
    }
